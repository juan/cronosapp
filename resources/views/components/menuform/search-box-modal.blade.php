...
<div class="card lg:flex">
    ...
    <div class="relative w-40 mt-2" x-data="">
        <x-input_select_form
                x-on:change="$wire.set('{{$wireinputname}}','')"
                label="Opción" idbox="tipobusquedad" wireid="selectsor">
            @foreach($opcionSort as $key => $dataorder)
                <option value="{{$key}}">{{$dataorder}}</option>
            @endforeach
        </x-input_select_form>
    </div>
    ...
    <div x-data="{ isOpen: false, timeout: null }"
         ...
         class="flex items-center gap-2 p-3 border-t lg:border-t-0 lg:ltr:border-l lg:rtl:border-r
         border-divider">
        <span>Mostrar</span>
        <div @mouseover="isOpen = true" class="dropdown">
            <button

                    class="btn btn_outlined btn_secondary" type="button">
                {{$numpage}}
                <span class="ltr:ml-3 rtl:mr-3 la la-caret-down text-xl leading-none"></span>
            </button>
            <div class="absolute z-10 mt-2 w-16 origin-top-right divide-y rounded-md bg-white shadow-lg
            ...
            >
                <div class="py-1" role="none">
                    <a class="text-gray-700 block px-4 py-0.5 text-sm" role="menuitem" tabindex="-1"
                       @click="Livewire.emit('numofpage','10')"
                    >10
                    </a>
                </div>
                <div class="py-1" role="none">
                    <a class="text-gray-700 block px-4 py-0.5 text-sm" role="menuitem" tabindex="-1"
                       @click="Livewire.emit('numofpage','20')">20</a>
                </div>
                <div class="py-1" role="none">
                    <a class="text-gray-700 block px-4 py-0.5 text-sm" role="menuitem" tabindex="-1"
                       @click="Livewire.emit('numofpage','45')">45</a>
                </div>
                <div class="py-1" role="none">
                    <a class="text-gray-700 block px-4 py-0.5 text-sm" role="menuitem" tabindex="-1"
                       @click="Livewire.emit('numofpage','60')">60</a>
                </div>
            </div>
        </div>
        <span>items</span>
