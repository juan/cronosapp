    ...
    <div class="modal-dialog  w-9/12">
        <div class="modal-content w-full">
            <div class="modal-header w-full h-2">
                <h2 class="modal-title">Listado de usuarios</h2>
            ...
            <div class="modal-body ">
                ...
                <div class="flex flex-col w-full">
                    @if(count($listUsers)>0)
                        ...
                        <x-smalltable cabecera="DNI,Nombre,Apellido,Teléfono,Correo,Opciones"
                                      sortby="dni,name,lastname,phone" :apuntador="$apuntador">
                            @foreach($listUsers as $datauser)
                                <tr>

                                    <td class="p-1">{{$loop->iteration}}</td>
                                    <td class="text-social-facebook opacity-100  border-4">{{$datauser->dni}}</td>
                                    <td>{{$datauser->name}}</td>
                                    ...
                                    <td class="px-4">
                                        <x-opcion-table editon="yes" viewon="yes">
                                            <x-slot:funciowiew>
                                                loadUserInfo({{$datauser->id}},'view')
                                            </x-slot:funciowiew>
                                            <x-slot:funcioupdate>
                                                loadUserInfo({{$datauser->id}},'update')
                                            </x-slot:funcioupdate>
