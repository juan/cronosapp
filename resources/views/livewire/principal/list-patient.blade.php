    ...
    <div class="modal-dialog  w-8/12">
        <div class="modal-content w-full">
            <div class="modal-header w-full h-2">
                <h2 class="modal-title">Listado de pacientes</h2>
            ...
            <div class="modal-body ">
                ...
                <div class="flex flex-col w-full">
                    @if(count($listPatient)>0)
                        ...
                        <x-smalltable cabecera="DNI,Nombre,Apellido,Teléfono,Opciones"
                                      sortby="numberid_patient,name_patient,lastname_patient" :apuntador="$apuntador">
                            @foreach($listPatient as $datapatient)
                                <tr>

                                    <td class="p-1">{{$loop->iteration}}</td>
                                    <td class="text-social-facebook opacity-100  border-4">{{$datapatient->numberid_patient}}</td>
                                    <td>{{$datapatient->name_patient}}</td>
                                    <td>{{$datapatient->lastname_patient}}</td>
                                    <td>{{$datapatient->cellphone}}</td>
                                    <td class="px-4">
                                        <x-opcion-table editon="yes" viewon="yes">
                                            <x-slot:funciowiew>
                                                loadPatientInfo({{$datapatient->id}},'view')
                                            </x-slot:funciowiew>
                                            <x-slot:funcioupdate>
                                                loadPatientInfo({{$datapatient->id}},'update')
                                            </x-slot:funcioupdate>
