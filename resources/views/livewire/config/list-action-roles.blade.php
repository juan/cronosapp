        ...
        <div class="modal-content w-full">
            <div class="modal-header w-full h-2">
                ...
                <div class="relative">
                    ...
                    @can('EXPORT')
                        <div
                                wire:click.prevent="downloadlistPatient"
            ...
            <div class="modal-body ">
                ...
                <div class="flex flex-col w-full">
                    @if(count($listRoles)>0)
                        ...
                        <x-smalltable cabecera="Perfil,Permisos,Opciones"
                                      sortby="name_role" :apuntador="$apuntador">
                            @foreach($listRoles as $dataaccrole)
                                <tr>

                                    <td class="p-1">{{$loop->iteration}}</td>
                                    <td class="text-social-facebook opacity-100  border-4">{{$dataaccrole->name_role}}</td>
                                    <td class="p-1 text-center">
                                         <span class="inline-flex items-center rounded-full bg-fondopositivo
                                    px-2 py-1 text-xs font-body text-primary">
                                              {{ $dataaccrole->actions_count." / ".$countActions}}</span>
                                    </td>
                                    <td class="px-4">
                                        <x-opcion-table editon="yes" viewon="yes">
                                            <x-slot:funciowiew>
                                                loadActionRoleInfo({{$dataaccrole->id}},'view')
                                            </x-slot:funciowiew>
                                            <x-slot:funcioupdate>
                                                loadActionRoleInfo({{$dataaccrole->id}},'update')
                                            </x-slot:funcioupdate>
